...
echo '<button hx-post="'.$writer_uri.'" hx-trigger="click" hx-include="[name=\'csrf_token\']" hx-swap="none" name="sorting_products_dir" value="ASC" title="'.$lang['btn_sort_asc'].'" class="btn btn-default w-100">'.$icon['arrow_up'].'</button> ';
echo '<button hx-post="'.$writer_uri.'" hx-trigger="click" hx-include="[name=\'csrf_token\']" hx-swap="none" name="sorting_products_dir" value="DESC" title="'.$lang['btn_sort_desc'].'" class="btn btn-default w-100">'.$icon['arrow_down'].'</button>';
echo '</div>';
echo '</div>';
echo '</div>';
...
echo '<div id="checksMissingSKU" hx-get="'.$reader_uri.'?action=check_missing_sku" hx-trigger="load" class="border-0 p-0 m-0"></div>';
...
echo '<div class="list-group list-group-flush">';
echo '<button hx-post="'.$writer_uri.'" hx-target="#cacheDeleteResponse" hx-vals=\''.json_encode($vals).'\' hx-indicator=".htmx-indicator" class="list-group-item list-group-item-action d-flex justify-content-between align-items-center" name="products_cache" value="clear" title="'.$lang['btn_clear_cache'].'">';
echo '<span>'.$icon['trash'].' '.$lang['btn_delete_cache'].'</span>';
echo '<span id="cacheDeleteResponse" class=""></span>';
echo '</button>';
echo '<button hx-post="'.$writer_uri.'" hx-target="#cacheUpdateResponse" hx-vals=\''.json_encode($vals).'\' hx-indicator=".htmx-indicator" class="list-group-item list-group-item-action d-flex justify-content-between align-items-center" name="products_cache" value="update" title="'.$lang['btn_clear_cache'].'">';
echo '<span>'.$icon['arrow_clockwise'].' '.$lang['btn_update'].'</span>';
echo '<span id="cacheUpdateResponse" class=""></span>';
echo '</button>';
echo '</div>';
echo '</div>';
