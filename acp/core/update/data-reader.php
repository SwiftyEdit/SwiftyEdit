...
if($_GET['action'] == 'check_download') {
    ...
    if(is_dir($extract_dir)) {
        ...
        foreach($get_downloads as $download) {
            ...
            echo '<button class="btn btn-default text-success" hx-post="/admin/update/write/" hx-vals=\''.json_encode($hx_vals).'\' hx-target="#updateResponse" name="install_update" value="'.$download.'">'.$icon['sync_alt'].' Install</button>';
            echo '<button class="btn btn-danger" hx-post="/admin/update/write/" hx-vals=\''.json_encode($hx_vals).'\' hx-target="#updateResponse" name="remove_download" value="'.$download.'">'.$icon['trash_alt'].' Delete</button>';
            echo '</div>';
            echo '</div>';
        }
