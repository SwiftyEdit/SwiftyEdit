...
if($mode == 'new') {
    $submit_btn = '<button type="submit" hx-post="/admin/events/write/" hx-target="#formResponse" hx-swap="innerHTML" class="btn btn-success w-100" name="save_post" value="save">'.$lang['save'].'</button';
} else {
    $submit_btn = '<button type="submit" hx-post="/admin/events/write/" hx-target="#formResponse" hx-swap="innerHTML" class="btn btn-success w-100" name="save_post" value="update">'.$lang['update'].'</button';
}
