...
$sql_count_pages = "
SELECT count(*) AS 'All',
(SELECT count(*) FROM se_pages WHERE page_status LIKE '%public%' ) AS 'public', 
(SELECT count(*) FROM se_pages WHERE page_status LIKE '%ghost%' ) AS 'ghost', 
(SELECT count(*) FROM se_pages WHERE page_status LIKE '%draft%' ) AS 'draft', 
(SELECT count(*) FROM se_pages WHERE page_status LIKE '%private%' ) AS 'private', 
(SELECT count(*) FROM se_pages WHERE page_redirect <> '' AND page_redirect IS NOT NULL ) AS 'redirect'
FROM se_pages";
...
if(isset($_SESSION['kw_filter']) AND $_SESSION['kw_filter'] != "") {
	...
	foreach($all_filter as $f) {
		if($_REQUEST['rm_keyword'] == "$f") { continue; }
		if($f == "") { continue; }
		$set_keyword_filter .= "(page_meta_keywords like '%$f%' OR page_title like '%$f%' OR page_linkname like '%$f%' OR page_content like '%$f%') AND";
...
if($subinc == "pages.list" OR $subinc == "pages.snippets") {
	...
	foreach($lang_codes as $lang_code) {
		...
		$lang_filter_box .= '<a href="acp.php?tn=pages&sub='.$sub.'&switchLang='.$lang_code.'" class="btn btn-sm btn-default '.$this_btn_status.'">'.$lang_code.'</a>';
	}
