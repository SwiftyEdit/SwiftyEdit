...
if(isset($_REQUEST['selected_folder'])) {
	$_SESSION['disk'] = se_filter_filepath($_REQUEST['selected_folder']);
}
...
if(isset($_POST['delete'])) {
	...
	$get_file_data = se_get_media_data_by_id($deleteFile);
...
if(isset($_POST['rebuild']) && ($_POST['rebuild'] == 'database')) {
	...
	foreach($a_files as $file) {
		...
		$mediaData = $db_content->get("se_media", ["media_file"], [
			"media_file" => "$filename"
		]);
	...
	$storedFiles = $db_content->select("se_media", '*', [
		"media_file[~]" => "$disk"
	]);
	
	foreach($storedFiles as $k) {
		if(!is_file($k['media_file'])) {
			se_delete_media_data($k['media_file']);
			$cnt_files_removed++;
				if((time()-$rebuild_start) > 5) {
					$incomplete = TRUE;
					break;
	...
	$missing_thumb = $db_content->select('se_media', '*', [
		"AND" => [
			'OR' => [
	...
	if(count($missing_thumb)>0) {
		foreach($missing_thumb as $row) {
            ...
            $tmb_dir = $img_tmb_path;
            $tmb_dir_year = $tmb_dir.'/'.date('Y',$row['media_upload_time']);
            $tmb_destination = $tmb_dir_year.'/'.date('m',$row['media_upload_time']);

			$image = $row['media_file'];
			$tmb_name = md5($image).'.jpg';
			$store_tmb_name = $tmb_destination.'/'.$tmb_name;

			se_create_thumbnail($row['media_file'], $tmb_name, $tmb_destination, $se_prefs['prefs_maxtmbwidth'], $se_prefs['prefs_maxtmbheight'], 80);
	...
	$missing_rows = $db_content->select('se_media', '*', [
		"AND" => [
...
if($_SESSION['media_filter'] != "") {
	...
	foreach($all_filter as $f) {
		if($_REQUEST['rm_keyword'] == "$f") { continue; }
		if($f == "") { continue; }
		$btn_remove_keyword .= '<a class="btn btn-default btn-sm" href="acp.php?tn=filebrowser&sub=browse&rm_keyword='.$f.'">'.$icon['times_circle'].' '.$f.'</a> ';
	}
}
...
$sql_cnt = "SELECT count(*) AS 'all' FROM se_media WHERE media_file LIKE '%$disk%' ".$_SESSION['media_filter_string'];
...
$sql = "SELECT * FROM se_media $where_sql ".$_SESSION['media_filter_string']." $order_sql $limit_sql";
$get_files = $db_content->query($sql)->fetchAll(PDO::FETCH_ASSOC);
$get_files = se_unique_multi_array($get_files,'media_file');

$cnt_pages = ceil($nbr_of_files/$files_per_page);
$cnt_get_files = count($get_files);

$pag_backlink = '<li class="page-item"><a class="page-link '.$disable_prev_start.'" href="acp.php?tn=filebrowser&start='.$prev_start.'">'.$icon['caret_left'].'</a></li>';
$pag_forwardlink = '<li class="page-item"><a class="page-link '.$disable_next_start.'" href="acp.php?tn=filebrowser&start='.$next_start.'">'.$icon['caret_right'].'</a></li>';
...
foreach($se_labels as $label) {
	...
	$label_filter_box .= '<a href="acp.php?tn=filebrowser&switchLabel='.$label['label_id'].'" class="btn btn-default btn-sm m-1 '.$this_btn_status.'">'.$label_title.'</a>';
