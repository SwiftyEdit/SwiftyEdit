...
function se_crawler($id='') {
	...
	$check_page = $se_base_url.$url;
	
	if(isset($_POST['start_index']) && $_POST['start_index'] != '') {
		$check_page = $_POST['start_index'];
		/* we are indexing our own pages only */
		$check_len = strlen($se_base_url);
		if(substr($check_page, 0,$check_len) != $se_base_url) {
			return;
		}
	}
	

	$get_html = se_loadSourceCode($check_page);
	$links = se_get_links($get_html);
...
function se_get_html_data($html) {
...
function se_update_page_index($id) {
    ...
    if($get_html == '') {
        echo '<div class="alert alert-danger">can not read: '.$check_page.'</div>';
        return;
    }
	
	$html_data = se_get_html_data($get_html);
	...
	while(strpos($get_html,'  ')!==false) $get_html = preg_replace("/ {2}/", " ", $get_html);
...
function se_update_bulk_page_index($num=5) {
	...
	foreach($items as $item) {
		$update = se_update_page_index($item['page_id']);
	}
...
function se_remove_page_from_index($id) {
	$dbh = new PDO("sqlite:".INDEX_DB);
...
function se_get_nbr_of_indexed_pages() {
...
function se_delete_excludes($id): void
{
...
function se_write_exclude_elements($element,$attribute): void
{
...
function se_get_exclude_elements() {
	$dbh = new PDO("sqlite:".INDEX_DB);
...
function se_write_exclude_url($url) {
