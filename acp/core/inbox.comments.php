...
$page_comments = $db_content->select("se_comments", "comment_relation_id",[
				"AND" => [
				"comment_type" => "p"
			]
]);
...
$all_pages = $db_content->select("se_pages", ["page_id","page_title"]);
...
$blog_comments = $db_content->select("se_comments", "comment_relation_id",[
				"AND" => [
				"comment_type" => "b"
			]
]);
...
$all_posts = $db_posts->select("se_posts", ["post_id","post_title"]);
...
$get_comments = se_get_comments($comments_start,$comments_limit,$cf_filter);
...
for($i=0;$i<$cnt_comments;$i++) {
	...
	echo '<form class="form-inline" action="?tn=inbox&sub=comments#comid'.$comment_id.'" method="POST">';
	echo '<button type="submit" class="btn btn-sm w-100 btn-default" name="editid" value="'.$comment_id.'">'.$lang['edit'].'</button>';
	...
	echo '<form class="form-inline" action="?tn=inbox&sub=comments" method="POST">';
	...
	echo '<div class="col-md-1">';	
	echo '<form class="form-inline" action="?tn=inbox&sub=comments" method="POST">';
	echo '<button type="submit" class="btn btn-sm w-100 btn-default text-danger" name="delid" value="'.$comment_id.'">'.$icon['trash_alt'].'</button>';
	echo $hidden_csrf_token;
	echo '</form>';
	echo '</div>';
	echo '</div>';
	...
	if($get_comments[$i]['comment_id'] == $editid) {
		...
		echo '<form action="?tn=inbox&sub=comments#comid'.$comment_id.'" method="POST">';
