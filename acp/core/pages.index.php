...
if(isset($_POST['del_exclude']) && is_numeric($_POST['del_exclude'])) {
	...
	se_delete_excludes($del_exclude);
}

if(isset($_POST['add_exclude_url'])) {
	se_write_exclude_url($_POST['exclude_url']);
}

if(isset($_POST['add_excludes'])) {
	se_write_exclude_elements($_POST['exclude_element'],$_POST['exclude_attribute']);
}


$exclude_items = se_get_exclude_elements();
$exclude_urls = se_get_exclude_urls();
...
if(isset($_POST['start_index_from']) && $_POST['start_index_from'] != '') {
	se_crawler($_POST['start_index_from']);
}
...
$cnt_all_indexed_entries = se_get_nbr_of_indexed_pages();
...
echo '<button class="btn btn-default '.$active_btn25.'" name="limit_index_items" value="25">25</button>';
echo '<button class="btn btn-default '.$active_btn50.'" name="limit_index_items" value="50">50</button>';
echo '<button class="btn btn-default '.$active_btn100.'" name="limit_index_items" value="100">100</button>';
echo '<button class="btn btn-default '.$active_btn0.'" name="limit_index_items" value="0">All (slow)</button>';
echo '</div>';
...
echo '<h5>Start</h5>';
echo '<form action="acp.php?tn=pages&sub=pages-index" method="POST">';
echo '<div class="row">';
echo '<div class="col-8">';
echo '<input type="text" class="form-control" name="start_index" value="'.$se_base_url.'">';
echo '</div>';
echo '<div class="col-4">';
echo '<button type="submit" name="start" class="btn btn-success w-100">'.$icon['sitemap'].'</button>';
echo '<input type="hidden" name="csrf_token" value="'.$_SESSION['token'].'">';
echo '</div>';
echo '</div>';
echo '</form>';
...
echo '<button name="start_index_bulk" class="btn btn-success">'.$icon['sitemap'].' '.$lang['btn_bulk_index'].'</button>';
echo '<button name="start_update_bulk" class="btn btn-success">'.$icon['sync_alt'].' '.$lang['btn_bulk_update'].'</button>';
echo '</div>';
...
echo '<input type="submit" name="add_excludes" value="'.$lang['save'].'" class="btn btn-success w-100">';
echo '<input type="hidden" name="csrf_token" value="'.$_SESSION['token'].'">';
echo '</div>';
echo '</div>';
echo '</div>';
...
echo '<div class="form-group">';
echo '<label class="d-block"><br></label>';
echo '<input type="submit" name="add_exclude_url" value="'.$lang['save'].'" class="btn btn-success w-100">';
echo '<input type="hidden" name="csrf_token" value="'.$_SESSION['token'].'">';
echo '</div>';
echo '</div>';
echo '</div>';
