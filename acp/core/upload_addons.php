...
if(!is_writable(SE_CONTENT.'/modules/')) {
...
if(isset($_POST['install_uploaded_plg'])) {
	...
	if(is_array($all_files)) {
		...
		foreach($all_files as $f) {
			...
			$i++;
			$target = SE_CONTENT."/plugins/" . substr($f, strlen("../upload/plugins/extract/$extracted/"));
...
if(isset($_POST['install_uploaded_mod'])) {
	...
	if(is_dir("../upload/modules/extract/$extracted"))	{
		$all_files = se_scandir_rec("../upload/modules/extract/$extracted");
	} else {
	...
	if(is_array($all_files)) {
		foreach($all_files as $f) {
			...
			$i++;
			$target = SE_CONTENT."/modules/" . substr($f, strlen("../upload/modules/extract/$extracted/"));
			$status = copy_recursive("$f","$target");
		...
		if(count($all_uploads) < 1) {
			...
			foreach($all_uploads as $upload) {
				...
				if($this_pathinfo['dirname'] == '../upload/modules') {
					...
					echo '<form action="?tn=moduls&sub=u" method="POST">';
					echo '<button class="btn btn-primary text-success" type="submit" name="install_uploaded_mod" value="'.$this_pathinfo['basename'].'">Install</button>';
					echo '<button class="btn btn-primary text-danger" type="submit" name="delete_uploaded_zip" value="'.$this_pathinfo['basename'].'">Remove</button>';
					...
					echo '<form action="?tn=moduls&sub=u" method="POST">';
					echo '<button class="btn btn-primary text-success" type="submit" name="install_uploaded_plg" value="'.$this_pathinfo['basename'].'">Install</button>';
					echo '<button class="btn btn-primary text-danger" type="submit" name="delete_uploaded_zip" value="'.$this_pathinfo['basename'].'">Remove</button>';
					...
					echo '<form action="?tn=moduls&sub=u" method="POST">';
					echo '<button class="btn btn-primary text-success" type="submit" name="install_uploaded_tpl" value="'.$this_pathinfo['basename'].'">Install</button>';
					echo '<button class="btn btn-primary text-danger" type="submit" name="delete_uploaded_zip" value="'.$this_pathinfo['basename'].'">Remove</button>';
